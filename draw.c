...
//front
  ...
  //back
  add_polygon(polygons, x0, y0, z1, x0, y0, z1, x0, y1, z1);
...
void add_sphere( struct matrix * edges, 
  ...
  longStart = 0;
  longStop = step;
  double x0,y0,z0,x1,y2,z2,x3,y3,z3;
  ...
  for ( lat = latStart; lat < latStop; lat++ ) {
    for ( longt = longStart; longt <= longStop; longt++ ) {
      ...
      index1 = (index + 1) % points -> lastcol;
      index2 = (index + step) % points -> lastcol;
      index3 = (index + step - 1) % points -> lastcol;
      /*
      ...
      x1 = points -> m[0][index1];
      y1 = points -> m[1][index1];
      z1 = points -> m[2][index1];

      x2 = points -> m[0][index2];
      y2 = points -> m[1][index2];
      z2 = points -> m[2][index2];

      x3 = points -> m[0][index3];
      y3 = points -> m[1][index3];
      z3 = points -> m[2][index3];

      add_polygon(edges, x0, y0, z0, x1, y1, z1, x2, y2, z2);
      add_polygon(edges, x0, y0, z0, x2, y2, z2, x3, y3, z3);
    }
  }
  free_matrix(points);
}
